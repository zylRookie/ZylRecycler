 ...
 */
public abstract class ZylAdapter<T> extends RecyclerView.Adapter<ZylRViewHolder> {

    private Context mContext;
    private int mDefaultLayoutId = 0;
    private List<T> mData;
    private ZylHAFAdapter mHeaderAndFooterAdapter;
     ...
     */
    public ZylAdapter(Context context, List<T> data) {
        mContext = context;
        mData = data;
    }
     ...
     */
    public ZylAdapter(Context context, List<T> data, int defaultLayoutId) {
        this(context, data);
    ...
    public int getItemViewType(int position) {
        if (mDefaultLayoutId == 0) {
            throw new RuntimeException("请在 " + this.getClass().getSimpleName() + " 中重写 getItemViewType 方法返回布局资源 id，或者使用 " + this.getClass().getSimpleName() + " 三个参数的构造方法 ZylAdapter(Context context, int defaultLayoutId, List<T> data)");
        }
        return mDefaultLayoutId;
    }

    @Override
    public ZylRViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ZylRViewHolder holder = new ZylRViewHolder(mContext, View.inflate(mContext, viewType, null));
    ...
    @Override
    public void onBindViewHolder(ZylRViewHolder holder, int position) {
        convert(holder, mData.get(position), position);
    }

//    @Override
//    public void onBindViewHolder(ZylRViewHolder holder, int position, List<Object> payloads) {
//        convert(holder, mData.get(position), position);
//    }
    ...
    public abstract void convert(ZylRViewHolder helper, T item, int position);
    ...
    public ZylHAFAdapter getHeaderAndFooterAdapter() {
        if (mHeaderAndFooterAdapter == null) {
            synchronized (ZylHAFAdapter.class) {
                if (mHeaderAndFooterAdapter == null) {
                    mHeaderAndFooterAdapter = new ZylHAFAdapter(this);
                }
            }
        }
