...
/**
 * Created by yongliangzhao on 2017/10/12.
 */
 ...
 */
public class ZylRecyclerView extends RecyclerView {
    ...
    public ZylRecyclerView(Context context) {
        super(context, null);
    }

    public ZylRecyclerView(Context context, AttributeSet attrs) {
        super(context, attrs);

        mContext = context;

        /*================== 获取自定义属性 begin ==================*/
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.ZylRecyclerView);
        for (int i = 0; i < typedArray.getIndexCount(); i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.ZylRecyclerView_rv_type) {
                type = typedArray.getInt(attr, 0);
            } else if (attr == R.styleable.ZylRecyclerView_rv_orientation) {
                orientation = typedArray.getInt(attr, 0);
            } else if (attr == R.styleable.ZylRecyclerView_rv_column) {
                column = typedArray.getInt(attr, 1);
            } else if (attr == R.styleable.ZylRecyclerView_rv_divider_size) {
                dividerSize = (int) typedArray.getDimension(attr, 0f);
            } else if (attr == R.styleable.ZylRecyclerView_rv_divider_drawable) {
                dividerDrawable = typedArray.getDrawable(attr);
            } else if (attr == R.styleable.ZylRecyclerView_rv_divider_color) {
                dividerColor = typedArray.getColor(attr, Color.BLACK);
            } else if (attr == R.styleable.ZylRecyclerView_rv_default_animator_open) {
    ...
    /**
     * 提醒ZylRecyclerView，当前的列表样式已经更改（一般是用代码动态修改了type和orientation后调用，如果修改了分割线样式，也需要调用该方法进行刷新）
     */
    ...
    class RecyclerViewListener extends RecyclerView.OnScrollListener {
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            ...
            if (move && newState == RecyclerView.SCROLL_STATE_IDLE) {
                ...
                if (0 <= n && n < ZylRecyclerView.this.getChildCount()) {
                    int top = ZylRecyclerView.this.getChildAt(n).getTop();
                    ZylRecyclerView.this.smoothScrollBy(0, top);
                }
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            ...
            if (move) {
                ...
                if (0 <= n && n < ZylRecyclerView.this.getChildCount()) {
                    int top = ZylRecyclerView.this.getChildAt(n).getTop();
                    ZylRecyclerView.this.scrollBy(0, top);
                }
            }
        }
    }

    /*================== ZylRecyclerView的滚动事件拓展 begin ==================*/
    ...
    /**
     * ZylRecyclerView的滚动事件拓展（原滚动事件被用于平滑滚动）
     */
    ...
    class LQRItemDecoration extends ItemDecoration {
        ...
        public LQRItemDecoration(Context context, int orientation, int dividerSize, int dividerColor, Drawable dividerDrawable) {
            ...
            if (dividerDrawable == null) {
                ...
                if (mDividerSize == 0) {
                    if (mOrientation == ZylRecyclerView.ORIENTATION_VERTICAL) {
